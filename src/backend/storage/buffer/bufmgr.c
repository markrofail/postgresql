...
MarkBufferDirtyHint(Buffer buffer, bool buffer_std)
{
		...
		(BM_DIRTY | BM_JUST_DIRTIED))
	{
		...
		if (XLogHintBitIsNeeded())
		{
			 ...
			 * they are not restored.  WAL-skipped relations should never have
			 ...
			 * during a checkpoint for permanent, non-WAL-skipped relations.
			 ...
			 * encryption does not support WAL-skip relations.)  We must update
			 * the page LSN even if the page with the hint bit change is later
			 * overwritten in the file system with an earlier version of the
			 * page during crash recovery.
			 *
			...
			if (XLogRecPtrIsInvalid(lsn) && FileEncryptionEnabled)
			{
				 ...
				 * permanent, non-WAL-skip relations.  For non-permanent
				 * relations, they crash recover as empty.  For WAL-skip
				 * relations, they recover with their original contents, so
				 * that works too.
				 */
