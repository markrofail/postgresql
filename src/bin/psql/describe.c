...
listTables(const char *tabtypes, const char *pattern, bool verbose, bool showSystem)
{
	...
	if (verbose)
	{
		...
		if (pset.sversion >= 140000)
			appendPQExpBuffer(&buf,
							  ",\n CASE"
							  " WHEN c.relkind in ("CppAsString2(RELKIND_INDEX)") THEN"
							  "		pg_catalog.pg_size_pretty(pg_catalog.pg_index_size(c.oid))"
							  " WHEN c.relkind in ("CppAsString2(RELKIND_SEQUENCE)") THEN"
							  "		pg_catalog.pg_size_pretty(pg_catalog.pg_sequence_size(c.oid))"
							  " ELSE"
